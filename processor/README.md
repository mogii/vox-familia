...
对着摄像头一件件介绍宝宝闲置，说「**下一件**」切下一样，想查新品参考价就顺嘴说「**查原价**」。
手机分享视频到 Mac 上跑着的这个服务，它自动转写、切段、挑图、用大模型提价格写描述、
联网搜参考价，然后你在 iPhone 浏览器里检查、改、勾图、给每张图配一句话，导出到相册——
之后随手发到微信群里就行。

所有处理都在你 Mac 本地：Whisper 跑在本地，**视频不离开你电脑**；只有每件那段口述
文字会发给大模型整理，说了「查原价」的那件会联网搜一次。
...
模型（`small` 约几百 MB）。成品图印中文用 Mac 自带字体。

`.env` 必填：

- `ANTHROPIC_API_KEY` — 大模型 key（提价格 / 写描述，费用很小）。可用官方 Claude，
...
1. **录视频**：iPhone 录像，一件件介绍，说「下一件」切下一样。可以临时改价（脚本会取你
   **最后说定**的那个）；想让买家看到「原价对比」就对那件说一声「查原价」。
2. **触发处理**：照片里找到这条视频 → 分享 → 选「处理闲置」。手机会推一条通知"已上传"。
3. **去看结果**：iPhone 浏览器里那个一直开着的页面下拉刷新（或等它自己刷新）。处理
   完会看到一条条商品。
4. **审一下**：每件可以改标题/价格/原价/成色/描述；点缩略图勾上/取消想要的照片；
   给勾中的每张照片配一句说明（可空）。
5. **导出**：每件下面两个按钮——
   - 「导出原图到相册」：把你勾的几张原图存进相册
   - 「导出成品长图」：把勾中的几张**合成一张长图**存进相册——首图下方一块白卡印
     标题、价格、~~原价~~（删除线）、成色、描述；其余每张图下面印你配的那句话
...
录视频时对那件说了「查原价」，管线就用 Kimi 联网搜它的**新品参考价**，记进 listing：

- `marketSearched` — 有没有搜过。`true` + 价格为空 = 「搜了但没搜到」；`false` = 没搜。
- `marketPriceText` — 给人看的参考价文本（如 `$169.99–$199.99`），网页里可改可清空。
- `marketSource` — 参考来源链接。

成品长图上有参考价就会以删除线印在你售价旁边，买家一眼看出省多少。老款未必有精确
对应价，都当参考。
...
├── items/1/...jpg      ...
└── posters/0.jpg       第 1 件的成品长图（按需生成；改了字或图会自动重新生成）
```
...
- **成品图里中文是方块**：Mac 上没找到 CJK 字体。`.env` 里 `POSTER_FONT` 指到一个
