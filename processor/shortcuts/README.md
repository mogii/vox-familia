...
## Shortcut 1：处理闲置（上传视频或照片）

**作用**：Photos 里选中刚录的视频（或一张/多张照片）→ Share → 点这条捷径 →
自动上传到 Mac。

- **传视频**：跑完整管线（转写、切段、挑图、AI 提价…）。
- **传照片**：一次分享的几张算**一件商品**（同一件东西的多角度图），立即出现在
  网页里，照片全勾好，标题/价格/描述你在网页里填。要发多件就分多次分享。
  HEIC 会自动转 JPEG、自动摆正，不用预处理。
...
- 点 **Receive** 那行，接收类型勾 **Images**（图片）和 **Media**（媒体）
  （保留 **Files** 也行）
